...
void User::unSubscribe(Video *video){
    Node<Video*>*sil=subscriptions.findNode(video);

    Node<Video*>*p=subscriptions.findPrev(sil->getData());
...
void User::removeFriend(User * user){
    Node<User*>*sil=friends.findNode(user);
    User* sil2=this;
    User*tut=user;

    this->getFriends()->deleteNode(this->getFriends()->findPrev(user));
    tut->getFriends()->deleteNode(tut->getFriends()->findPrev(this));


    }

LinkedList<Video* > * User::getSubscriptions(){return &subscriptions;}
LinkedList< User* > * User::getFriends(){return &friends;}
